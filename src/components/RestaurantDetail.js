...
function RestaurantDetail() {
  ...
  React.useLayoutEffect(() => {
    axiosInstance
      .get(`/api/restaurant/${params.slug}`)
  ...
  dataProducts
    ? (document.title = "Restaurant | " + dataProducts.name)
  ...
  return (
    <React.Fragment>
      ...
      >
          ...
          <Item>
            ...
            <p className="vote">
              ...
              <span style={{ color: "red", fontWeight: "bold" }}>
                ({dataProducts ? dataProducts.vote.length : "0"})
              </span>
              <a href="#danhgia" style={{ color: "blue" }}>
                &nbsp;xem đánh giá
              </a>
            </p>
            <p style={{ display: "flex" }}>
              ...
              {dataProducts
                ? dataProducts.time_start + " - " + dataProducts.time_end
                : "00:00:00 - 11:59:59"}
            </p>
            <p style={{ display: "flex", color: "gray" }}>
              <MonetizationOnRoundedIcon />
              {dataProducts
                ? dataProducts.menu[0].price +
                  " - " +
                  dataProducts.menu[dataProducts.menu.length - 1].price
                : "0"}
            </p>
            <p style={{ display: "flex", color: "red" }}>
              <LocalPhoneRoundedIcon />{" "}
              {dataProducts
                ? dataProducts.user.info
                  ? dataProducts.user.info.phone
                  : ""
                : ""}
            </p>
          </Item>
        ...
        <Menu
          eating={dataProducts.menu}
          restaurantId={dataProducts.id}
          table={dataProducts.tables}
          isHours={isHours}
        />
        <Vote
          votes={dataProducts.vote}
          restaurant_id={dataProducts.id}
          load={handleClick}
        />
        <div id="map">
          <MapBox />
