...
export default function Quanlty({ btn, value, price, func, idCart, prdId }) {
  ...
  const updateCart = (quanlitys) => {
    axiosInstance
      .put("http://localhost:8000/api/cart/" + idCart, { quanlity: quanlitys })
      ...
      axiosInstance
        ...
        .then((response) => {
          let dom = (document.getElementById(
            "badge_header"
          ).children[1].innerText = response["data"] - 1);
          console.log({ dom });
        });
    }
  };
    ...
    <React.Fragment>
      <Box sx={{ display: "flex", alignItems: "flex-end" }}>
        ...
        <TextField
          ...
          sx={{ input: { textAlign: "center", maxWidth: "100px" } }}
          id="quanlityPrdDt"
          label="Quanlity"
          value={qltCount}
          variant="standard"
      ...
      {!btn && (
        <Button
          sx={{ mt: "40px", p: "15px 30px", background: "#f51167" }}
          color="error"
