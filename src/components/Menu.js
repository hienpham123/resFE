...
function Menu({ eating = [], restaurantId = false, table = false, isHours }) {
  ...
  useEffect(() => {
    if (table) {
      ...
      if (localStorage.getItem("tableres" + restaurantId)) {
        setSelectedValue(
          JSON.parse(localStorage.getItem("tableres" + restaurantId))
        );
      }
    }
  }, [table]);
  ...
  const handleGetData = () => {
    ...
    bodyFormData.set(
      "arrival_time",
      localStorage.getItem("timeS" + restaurantId)
    );
    axiosAuth
      .post("api/order", bodyFormData)
      ...
      .then(function (data) {
        if (data) {
          ...
          axiosAuth
            .post("api/order-detail", dataOrderDetail)
            .then((response) => response)
            .then(function (data) {
              if (data) {
                sendNotification({
    ...
    data.forEach((e) => {
      ...
      eating.forEach((val) => {
        if (e.id === val.id) {
          ...
          menuStg.sumPriceD += Math.round(
            (val.price - (val.price * val.discount) / 100) * e.quanlity
          );
        }
      });
    });
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            <TableBody>
              ...
              >
                ...
                <TableCell
                  align="right"
                  sx={{ color: "#f51167", fontWeight: "bold" }}
                >
                  {menuStg.count}
                </TableCell>
              </TableRow>
              ...
              >
                ...
                >
                  <SpDialog
                    ...
                    datas={hoursSelected.length > 0 ? hoursSelected : [0]}
                  />
                </TableCell>
              </TableRow>
            </TableBody>
