...
export default function AddProduct() {
  ...
  React.useEffect(() => {
    params &&
      axios
        .get(`http://localhost:8000/api/this-product/${params}`)
        .catch((error) => console.log(error))
        .then((res) => {
          setPrd(res["data"]);
          let data = res["data"];
  ...
  const handleDeleteImg = (index, e) => {
    axios
      .delete(`http://localhost:8000/api/images/${e}`)
  ...
  return (
    <React.Fragment>
      ...
      <h5 style={{ marginBottom: "9px", fontSize: "1.25rem" }}>
        Product information
      </h5>
      ...
      <Box
        sx={{ "& > :not(style)": { m: 1, width: "100%" }, textAlign: "center" }}
      >
        <Button
          ...
          product["images"].map((value, key) => {
            return (
              ...
              >
                <img
                  style={{ width: "95%" }}
                  src={"http://localhost:8000/" + value["path"]}
                />
                <Button
                  variant="contained"
                  onClick={(e) => {
                    handleDeleteImg(key, value["id"]);
                  }}
                >
                  Delete
                </Button>
      ...
      <Box sx={{ width: "100%", textAlign: "center", my: "20px" }}>
        <Button
          onClick={update}
          variant="contained"
          sx={{ p: "15px 50px", fontSize: "1.25rem" }}
        >
          Upload
        </Button>
      </Box>
