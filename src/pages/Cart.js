...
export default function CustomizedTables() {
  ...
  const deleteCart = (id) => {
    axiosInstance
      ...
      .then((response) => {
        ...
        if (response.data == 1) {
          let count =
            document.getElementById("badge_header").children[1].innerText;
          let dom = (document.getElementById(
            "badge_header"
          ).children[1].innerText = count - 1);
  ...
  const btn = (id) => {
    return (
      <Button variant="contained" onClick={() => deleteCart(id)} color="error">
        Delete
      </Button>
    );
  };
  ...
  React.useEffect(() => {
    if (user_id) {
      axiosAuth
        .get(`http://localhost:8000/api/cart-user/${user_id}`)
        .catch((error) => console.log(error))
  ...
  cart.map((data, index) => {
    ...
    rows.push(
      createData(
        <CardCart
          key={index}
          image={"http://localhost:8000/" + data.images}
          name={data.name}
        />,
        <Quanlty
          value={data.quality}
